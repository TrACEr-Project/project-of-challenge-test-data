...
% need to add tensor toolbox and the function with Tikhonove regularization cp_wopt_reg.m
% addpath('/Users/ll/Documents/MATLAB/toolbox/Lu_imp_functions')
...
for i=1:nb_starts
    if i==1
        ...
        [Fac_X{i}, ~, out_X{i}] =cp_wopt_reg(X,W,nm_comp,lamda,'init','nvecs','lower',Low,'opt_option',optionsCP);
    else
        
        [Fac_X{i}, ~, out_X{i}] =cp_wopt_reg(X,W,nm_comp,lamda,'init','randn','lower',Low,'opt_option',optionsCP);
        
    end
    
    goodness_X1(i) = out_X{i}.ExitMsg;
    goodness_X(i,1) = (1-norm(tensor(times(X,W)-full(Fac_X{i})))^2/norm(tensor(times(X,W)))^2)*100;
    goodness_X(i,2) =norm(tensor(times(X,W)-full(Fac_X{i})))^2;
end
%% uniqueness test
% 0 -> NOT unique
% 1 -> Unique
